...
const ServerStatusDashboard = () => {
  ...
  const safeRequest = async (path: string, options: RequestInit = {}) => {
    ...
    try {
      ...
      // Vercel Proxy 404 handler
      if (res.status === 404) {
          if (trimmed === "404 File not found") {
              throw new Error(`Critical 404: Vercel routing failed to find the server file for '${url}'. Check your Vercel Build logs to ensure 'server/server.js' compiled successfully.`);
          }
          if (trimmed.startsWith('<!DOCTYPE') || trimmed.startsWith('<html')) {
              throw new Error(`HTML 404: The server returned an error page for '${url}'. This route might be missing from your Express app.`);
          }
          try {
            const json = JSON.parse(trimmed);
            throw new Error(`API 404: ${json.error || 'Resource not found'}`);
          } catch (e) {
            throw new Error(`Not Found (404) at ${url}. Response: ${trimmed.substring(0, 50)}...`);
          }
      }

      if (trimmed.startsWith('<!DOCTYPE') || trimmed.startsWith('<html')) {
          throw new Error(`Unexpected HTML response at ${url}. Possible fall-through to the SPA index.html.`);
      }
  ...
  const fetchStatus = async () => {
    try {
      ...
      setError(err.message === 'Failed to fetch' ? 'Connection Error: Unable to reach the backend.' : err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const runSeed = async () => {
    if (!window.confirm("Initialize database with factory data? This will overwrite current entries.")) return;
    setSeeding(true);
    try {
      await safeRequest('/admin/seed', { method: 'POST' });
      await fetchStatus();
      setLastResponse({ path: '/admin/seed', status: 'SUCCESS', data: { message: "Database seeded successfully." } });
    } catch (err: any) {
  ...
  useEffect(() => {
    fetchStatus();
    const interval = setInterval(fetchStatus, 15000); // Poll every 15s to reduce serverless usage
    ...
    return (
      <div className="min-h-screen bg-[#020617] flex items-center justify-center text-slate-400 font-mono text-xs uppercase tracking-[0.3em]">
        <div className="flex flex-col items-center gap-4">
           ...
           <span className="animate-pulse">Handshaking with Cloud Service...</span>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-16">
          <div>
            ...
            <p className="text-slate-500 text-sm font-medium tracking-wide">Infrastructure Health & Real-time Database Diagnostics</p>
          </div>
          ...
          <div className={`flex items-center gap-3 px-6 py-2 rounded-full border transition-all duration-700 ${!isDbHealthy ? 'bg-red-500/10 border-red-500/30 text-red-500' : 'bg-emerald-500/10 border-emerald-500/30 text-emerald-400 shadow-[0_0_20px_rgba(16,185,129,0.1)]'}`}>
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.2em]">{isDbHealthy ? 'System Online' : 'Fault Detected'}</span>
          </div>
        </header>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
          <div className="lg:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-[#0f172a]/40 backdrop-blur-md border border-slate-800/60 p-7 rounded-[32px] shadow-2xl relative overflow-hidden group">
              <h3 className="text-slate-500 text-[10px] font-black mb-6 uppercase tracking-[0.3em]">Storage Instance</h3>
              <div className="space-y-4">
                <div className="bg-[#020617]/80 p-4 rounded-2xl border border-slate-800/50">
                  <p className="text-[9px] text-slate-500 font-mono mb-1 uppercase tracking-widest">Target Cluster</p>
                ...
                <div className="flex items-center justify-between px-1">
                   ...
                   <div className="text-right">
                      <span className="text-[9px] text-slate-600 font-mono uppercase block">Status Code</span>
                      <span className={`text-[10px] font-black ${status?.isSeeded ? 'text-blue-500' : 'text-amber-500'}`}>
                        {status?.isSeeded ? 'SEEDED' : 'CLEAN'}
                      </span>
                   </div>
                </div>
              </div>
            ...
            <div className="bg-[#0f172a]/40 backdrop-blur-md border border-slate-800/60 p-7 rounded-[32px] shadow-2xl relative overflow-hidden">
               <h3 className="text-slate-500 text-[10px] font-black mb-6 uppercase tracking-[0.3em]">Runtime Metrics</h3>
            ...
            <div className="bg-[#0f172a]/40 backdrop-blur-md border border-slate-800/60 p-8 rounded-[32px] shadow-2xl md:col-span-2 relative overflow-hidden">
               <h3 className="text-slate-500 text-[10px] font-black mb-8 uppercase tracking-[0.3em]">Data Volume Distribution</h3>
               <div className="grid grid-cols-2 sm:grid-cols-3 gap-6 relative z-10">
                  {[
                    ...
                    <div key={item.label} className="p-5 bg-[#020617]/40 border border-slate-800/40 rounded-[24px] hover:border-slate-600 transition-all transform hover:-translate-y-1">
          ...
          <div className="bg-[#0f172a]/40 backdrop-blur-md border border-slate-800/60 p-8 rounded-[32px] shadow-2xl flex flex-col h-full border-l-slate-700">
            <h3 className="text-slate-500 text-[10px] font-black mb-8 uppercase tracking-[0.3em]">API Command Center</h3>
            <div className="space-y-4 flex-grow">
              ...
              >
                <div>
                  <span className="text-[10px] font-black tracking-[0.1em] text-blue-500 block mb-1">INITIALIZE_SEED</span>
                  <span className="text-[11px] text-slate-400">Restore factory data model</span>
                </div>
        ...
        {(error || status?.dbError) && (
          <div className="mt-12 p-10 bg-red-950/20 border border-red-900/40 rounded-[40px] text-red-400 animate-in fade-in slide-in-from-bottom-8 shadow-2xl relative overflow-hidden">
             ...
             <div className="flex items-center gap-5 mb-6">
                ...
                <div>
                  <h2 className="text-xl font-black uppercase tracking-tight text-white">Critical System Fault</h2>
                  <p className="text-xs text-red-400/60 font-medium">Internal Negotiator Failure at the Protocol Layer.</p>
                </div>
             </div>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-xs text-slate-500">
                <div className="space-y-3">
                  <p className="font-black uppercase tracking-[0.2em] text-[10px] text-slate-300">Persistence Audit:</p>
                  <ul className="space-y-2 list-inside list-disc opacity-80">
                    <li>Verify the MONGODB_URI in Vercel project environment variables.</li>
                    <li>Ensure MongoDB Atlas Network Access is set to 0.0.0.0/0.</li>
                    <li>Check if the cluster has been suspended due to inactivity.</li>
                  </ul>
                </div>
                <div className="space-y-3">
                  <p className="font-black uppercase tracking-[0.2em] text-[10px] text-slate-300">Routing Diagnostics:</p>
                  <p className="opacity-80 leading-relaxed">
                    ...
                    rewrites explicitly point to <code className="text-blue-400 text-[10px]">/server/server.js</code>. 
